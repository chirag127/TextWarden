...
function getMockSuggestions(text) {
  ...
  const commonErrors = [
    ...
    { error: 'your welcome', correction: "you're welcome", type: 'grammar' },
    ...
    { error: 'due to the fact that', correction: "because", type: 'clarity' }
  ];
  ...
  commonErrors.forEach(({ error, correction, type }) => {
    ...
    while (index !== -1) {
      ...
      const afterChar = index + error.length >= text.length ? ' ' : text[index + error.length];
      
      if (/\s/.test(beforeChar) || /[.,;!?]/.test(beforeChar) || index === 0) {
        if (/\s/.test(afterChar) || /[.,;!?]/.test(afterChar) || index + error.length === text.length) {
          // Create a suggestion
